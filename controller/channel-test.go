...
func testChannel(channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	model.RecordConsumeLog(c, testUserID, model.RecordConsumeLogParams{
		...
		TokenName:        "Kiểm thử mô hình",
		Quota:            quota,
		Content:          "Kiểm thử mô hình",
...
func testAllChannels(notify bool) error {
	...
	if testAllChannelsRunning {
		...
		return errors.New("Thử nghiệm đang chạy")
	}
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			...
			if common.AutomaticDisableChannelEnabled && !shouldBanChannel {
				if milliseconds > disableThreshold {
					err := fmt.Errorf("Thời gian phản hồi %.2fs vượt quá ngưỡng %.2fs", float64(milliseconds)/1000.0, float64(disableThreshold)/1000.0)
		...
		if notify {
			service.NotifyRootUser(dto.NotifyTypeChannelTest, "Hoàn thành thử nghiệm kênh", "Tất cả thử nghiệm kênh đã hoàn thành")
		}
	})
	return nil
}
