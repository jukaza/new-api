...
var (
	...
	ErrFriendlyPasskeyNotFound = errors.New("Xác minh Passkey thất bại, vui lòng thử lại hoặc liên hệ quản trị viên")
)
...
func UpsertPasskeyCredential(credential *PasskeyCredential) error {
	if credential == nil {
		...
		return fmt.Errorf("Lưu Passkey thất bại, vui lòng thử lại")
	}
...
func DeletePasskeyByUserID(userID int) error {
	if userID == 0 {
		...
		return fmt.Errorf("Xóa thất bại, vui lòng thử lại")
	}
	// 使用Unscoped()进行硬删除，避免唯一索引冲突
