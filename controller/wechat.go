...
func getWeChatIdByCode(code string) (string, error) {
	if code == "" {
		return "", errors.New("Tham số không hợp lệ")
	}
	...
	if res.Data == "" {
		return "", errors.New("Mã xác minh sai hoặc đã hết hạn")
	}
	return res.Data, nil
}
...
func WeChatAuth(c *gin.Context) {
	if !common.WeChatAuthEnabled {
		c.JSON(http.StatusOK, gin.H{
			"message": "Quản trị viên chưa bật tính năng đăng nhập và đăng ký qua WeChat",
			"success": false,
		})
		return
	}
...
func WeChatBind(c *gin.Context) {
	...
	if model.IsWeChatIdAlreadyTaken(wechatId) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "Tài khoản WeChat này đã được liên kết",
		})
		return
	}
