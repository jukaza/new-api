...
func PullOllamaModel(baseURL, apiKey, modelName string) error {
	...
	if err != nil {
		return fmt.Errorf("tuần tự hóa yêu cầu thất bại: %v", err)
	}
	...
	response, err := client.Do(request)
	if err != nil {
		return fmt.Errorf("yêu cầu thất bại: %v", err)
	}
	defer response.Body.Close()

	if response.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(response.Body)
		return fmt.Errorf("tải mô hình thất bại %d: %s", response.StatusCode, string(body))
	}
...
func PullOllamaModelStream(baseURL, apiKey, modelName string, progressCallback func(OllamaPullResponse)) error {
	...
	for scanner.Scan() {
		...
		if strings.EqualFold(pullResponse.Status, "error") {
			return fmt.Errorf("tải mô hình thất bại: %s", strings.TrimSpace(line))
		}
	...
	if err := scanner.Err(); err != nil {
		return fmt.Errorf("đọc phản hồi dạng stream thất bại: %v", err)
	}

	if !successful {
		return fmt.Errorf("tải mô hình chưa hoàn thành: không nhận được trạng thái thành công")
	}
...
func DeleteOllamaModel(baseURL, apiKey, modelName string) error {
	...
	if response.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(response.Body)
		return fmt.Errorf("xóa mô hình thất bại %d: %s", response.StatusCode, string(body))
	}
...
func FetchOllamaVersion(baseURL, apiKey string) (string, error) {
	...
	if trimmedBase == "" {
		return "", fmt.Errorf("baseURL bị trống")
	}
	...
	if response.StatusCode != http.StatusOK {
		return "", fmt.Errorf("truy vấn phiên bản thất bại %d: %s", response.StatusCode, string(body))
	}
	...
	if versionResp.Version == "" {
		return "", fmt.Errorf("không có thông tin phiên bản được trả về")
	}
