...
func UpdateMidjourneyTaskBulk() {
	...
	for {
		...
		logger.LogInfo(ctx, fmt.Sprintf("Phát hiện số lượng nhiệm vụ chưa hoàn thành là: %v", len(tasks)))
		...
		for channelId, taskIds := range taskChannelM {
			logger.LogInfo(ctx, fmt.Sprintf("Kênh #%d có số lượng nhiệm vụ chưa hoàn thành là: %d", channelId, len(taskIds)))
			if len(taskIds) == 0 {
				continue
			}
			...
			for _, responseItem := range responseItems {
				...
				if useTime > 3600000 && task.Progress != "100%" {
					responseItem.FailReason = "Quá thời gian chờ tác vụ từ nhà cung cấp (hơn 1 giờ)"
				...
				if responseItem.VideoUrls != nil && len(responseItem.VideoUrls) > 0 {
					...
					if err != nil {
						logger.LogError(ctx, fmt.Sprintf("Tuần tự hóa VideoUrls thất bại: %v", err))
				...
				if (task.Progress != "100%" && responseItem.FailReason != "") || (task.Progress == "100%" && task.Status == "FAILURE") {
					logger.LogInfo(ctx, task.MjId+" tạo thất bại: "+task.FailReason)
					task.Progress = "100%"
					if task.Quota != 0 {
