...
func (t *TwoFA) Create() error {
	...
	if existing != nil {
		return errors.New("Người dùng đã thiết lập 2FA")
	}
...
func (t *TwoFA) Update() error {
	if t.Id == 0 {
		return errors.New("ID bản ghi 2FA không được để trống")
	}
...
func ValidateBackupCode(userId int, code string) (bool, error) {
	if !common.ValidateBackupCode(code) {
		return false, errors.New("Mã xác minh hoặc mã dự phòng không chính xác")
	}
...
func (t *TwoFA) ValidateTOTPAndUpdateUsage(code string) (bool, error) {
	// 检查是否被锁定
	if t.IsLocked() {
		return false, fmt.Errorf("Tài khoản đã bị khóa, vui lòng thử lại sau %v", t.LockedUntil.Format("2006-01-02 15:04:05"))
	}
	...
	if !common.ValidateTOTPCode(t.Secret, code) {
		...
		if err := t.IncrementFailedAttempts(); err != nil {
			common.SysLog("Cập nhật số lần thất bại 2FA thất bại: " + err.Error())
		}
		return false, nil
	}
	...
	if err := t.Update(); err != nil {
		common.SysLog("Cập nhật lịch sử sử dụng 2FA thất bại: " + err.Error())
	}
