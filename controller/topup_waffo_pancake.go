...
func RequestWaffoPancakeAmount(c *gin.Context) {
	...
	if req.Amount < int64(setting.WaffoPancakeMinTopUp) {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": fmt.Sprintf("Số lượng nạp không được nhỏ hơn %d", setting.WaffoPancakeMinTopUp)})
		return
	}
...
func SaveWaffoPancake(c *gin.Context) {
	...
	if err := service.SaveWaffoPancakeConfig(
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Lưu cấu hình Waffo Pancake thất bại store_id=%q product_id=%q error=%q",
			req.StoreID, req.ProductID, err.Error(),
		))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Lưu cấu hình thất bại"})
		return
	}
...
func CreateWaffoPancakePair(c *gin.Context) {
	...
	if merchantID == "" || privateKey == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Chưa cấu hình thông tin xác thực Waffo Pancake"})
		return
	}
	...
	if err != nil {
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Tạo cửa hàng và sản phẩm Waffo Pancake thất bại orphan_store=%t store_id=%q error=%q",
			orphan, func() string {
				if result == nil {
					return ""
				}
...
func ListWaffoPancakeCatalog(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Lấy cửa hàng và danh mục sản phẩm Waffo Pancake thất bại error=%q", err.Error(),
		))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Lấy danh mục thất bại"})
		return
	}
...
func CreateWaffoPancakeSubscriptionProduct(c *gin.Context) {
	...
	if strings.TrimSpace(req.Name) == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Tên gói không được để trống"})
		return
	}
	if strings.TrimSpace(req.Amount) == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Giá gói không được để trống"})
		return
	}
	...
	if merchantID == "" || privateKey == "" || storeID == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Waffo Pancake chưa cấu hình xong, vui lòng hoàn tất liên kết cổng thanh toán trong cài đặt thanh toán trước"})
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Tạo sản phẩm gói Waffo Pancake thất bại store_id=%q name=%q amount=%q error=%q",
			storeID, req.Name, req.Amount, err.Error(),
		))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Tạo sản phẩm gói thất bại"})
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"message": "success",
...
func ListWaffoPancakeSubscriptionProductOptions(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Lấy danh sách sản phẩm đăng ký Waffo Pancake thất bại store_id=%q error=%q", storeID, err.Error(),
		))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Lấy danh sách sản phẩm thất bại"})
		return
	}
...
func RequestWaffoPancakePay(c *gin.Context) {
	if !isWaffoPancakeTopUpEnabled() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Cấu hình Waffo Pancake không đầy đủ"})
		return
	}
	...
	if err != nil || user == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Người dùng không tồn tại"})
		return
	}
	...
	if err := topUp.Insert(); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Tạo đơn hàng nạp tiền Waffo Pancake thất bại user_id=%d trade_no=%s amount=%d error=%q", id, tradeNo, req.Amount, err.Error()))
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Tạo phiên thanh toán Waffo Pancake thất bại user_id=%d trade_no=%s error=%q", id, tradeNo, err.Error()))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Không thể khởi tạo thanh toán"})
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Tạo đơn hàng nạp tiền Waffo Pancake thành công user_id=%d trade_no=%s session_id=%s amount=%d money=%.2f", id, tradeNo, session.SessionID, req.Amount, payMoney))
...
func WaffoPancakeWebhook(c *gin.Context) {
	if !isWaffoPancakeWebhookEnabled() {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook bị từ chối reason=webhook_disabled path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
	...
	if expectedEnv != "test" && expectedEnv != "prod" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf(
			"Phần môi trường đường dẫn webhook Waffo Pancake không hợp lệ env=%q path=%q client_ip=%s",
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Đọc body request webhook Waffo Pancake thất bại path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
		c.String(http.StatusBadRequest, "bad request")
		return
	}

	signature := c.GetHeader("X-Waffo-Signature")
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Webhook Waffo Pancake nhận yêu cầu path=%q client_ip=%s signature=%q body=%q", c.Request.RequestURI, c.ClientIP(), signature, string(bodyBytes)))

	event, err := service.VerifyConfiguredWaffoPancakeWebhook(string(bodyBytes), signature)
	if err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Xác thực chữ ký webhook Waffo Pancake thất bại path=%q client_ip=%s signature=%q body=%q error=%q", c.Request.RequestURI, c.ClientIP(), signature, string(bodyBytes), err.Error()))
	...
	if !strings.EqualFold(strings.TrimSpace(event.Mode), expectedEnv) {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Môi trường webhook Waffo Pancake không khớp expected=%q actual_mode=%q event_id=%s order_id=%s client_ip=%s",
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Xác thực chữ ký webhook Waffo Pancake thành công event_type=%s event_id=%s order_id=%s client_ip=%s", event.NormalizedEventType(), event.ID, event.Data.OrderID, c.ClientIP()))
	...
	if isSubscription {
		...
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf(
				"Phân tích đơn hàng đăng ký webhook Waffo Pancake thất bại event_id=%s order_id=%s buyer_identity=%q client_ip=%s error=%q",
		...
		if err := model.CompleteSubscriptionOrder(tradeNo, string(bodyBytes), model.PaymentProviderWaffoPancake, ""); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Hoàn thành đăng ký Waffo Pancake thất bại trade_no=%s event_id=%s order_id=%s client_ip=%s error=%q", tradeNo, event.ID, event.Data.OrderID, c.ClientIP(), err.Error()))
			c.String(http.StatusInternalServerError, "retry")
			return
		}
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Hoàn thành đăng ký Waffo Pancake trade_no=%s event_id=%s order_id=%s client_ip=%s", tradeNo, event.ID, event.Data.OrderID, c.ClientIP()))
		c.String(http.StatusOK, "OK")
		return
	}
	...
	if err != nil {
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Phân tích đơn hàng webhook Waffo Pancake thất bại event_id=%s order_id=%s buyer_identity=%q client_ip=%s error=%q",
			event.ID, event.Data.OrderID, event.Data.MerchantProvidedBuyerIdentity, c.ClientIP(), err.Error(),
		))
		c.String(http.StatusOK, "OK")
		return
	...
	if err := model.RechargeWaffoPancake(tradeNo); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Xử lý nạp tiền Waffo Pancake thất bại trade_no=%s event_id=%s order_id=%s client_ip=%s error=%q", tradeNo, event.ID, event.Data.OrderID, c.ClientIP(), err.Error()))
		c.String(http.StatusInternalServerError, "retry")
		return
	}

	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Nạp tiền Waffo Pancake thành công trade_no=%s event_id=%s order_id=%s client_ip=%s", tradeNo, event.ID, event.Data.OrderID, c.ClientIP()))
	c.String(http.StatusOK, "OK")
}
