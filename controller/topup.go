...
func RequestEpay(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Lỗi tham số"})
		return
	}
	if req.Amount < getMinTopup() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": fmt.Sprintf("Số lượng nạp không được nhỏ hơn %d", getMinTopup())})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Lấy thông tin nhóm người dùng thất bại"})
		return
	}
	...
	if !operation_setting.ContainsPayMethod(req.PaymentMethod) {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Phương thức thanh toán không tồn tại"})
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Epay khởi tạo thanh toán thất bại user_id=%d trade_no=%s payment_method=%s amount=%d error=%q", id, tradeNo, req.PaymentMethod, req.Amount, err.Error()))
	...
	err = topUp.Insert()
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Epay tạo hóa đơn nạp tiền thất bại user_id=%d trade_no=%s payment_method=%s amount=%d error=%q", id, tradeNo, req.PaymentMethod, req.Amount, err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Tạo hóa đơn thất bại"})
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Epay tạo hóa đơn nạp tiền thành công user_id=%d trade_no=%s payment_method=%s amount=%d money=%.2f uri=%q params=%q", id, tradeNo, req.PaymentMethod, req.Amount, payMoney, uri, common.GetJsonString(params)))
...
func EpayNotify(c *gin.Context) {
	if !isEpayWebhookEnabled() {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay webhook bị từ chối reason=webhook_disabled path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
	...
	if c.Request.Method == "POST" {
		...
		if err := c.Request.ParseForm(); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Phân tích biểu mẫu POST Epay webhook thất bại path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
			_, _ = c.Writer.Write([]byte("fail"))
			return
		}
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Epay webhook nhận yêu cầu path=%q client_ip=%s method=%s params=%q", c.Request.RequestURI, c.ClientIP(), c.Request.Method, common.GetJsonString(params)))

	if len(params) == 0 {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay webhook tham số trống path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}
	client := GetEpayClient()
	if client == nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Epay client chưa được khởi tạo path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
	...
	if err == nil && verifyInfo.VerifyStatus {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Epay webhook verify signature success trade_no=%s callback_type=%s trade_status=%s client_ip=%s verify_info=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.TradeStatus, c.ClientIP(), common.GetJsonString(verifyInfo)))
		...
		if err != nil {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay webhook verify signature failed path=%q client_ip=%s verify_error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
		} else {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay webhook verify signature failed path=%q client_ip=%s verify_status=false", c.Request.RequestURI, c.ClientIP()))
		}
		return
	}
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			err := topUp.Update()
			if err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("Epay cập nhật hóa đơn nạp tiền thất bại trade_no=%s user_id=%d client_ip=%s error=%q topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), err.Error(), common.GetJsonString(topUp)))
				return
			}
			...
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("Epay nạp tiền thành công trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, topUp.Money, common.GetJsonString(topUp)))
			model.RecordTopupLog(topUp.UserId, fmt.Sprintf("Nạp tiền trực tuyến thành công, số tiền nạp: %v, số tiền thanh toán: %f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
		}
	} else {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Epay webhook ignore event trade_no=%s callback_type=%s trade_status=%s client_ip=%s verify_info=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.TradeStatus, c.ClientIP(), common.GetJsonString(verifyInfo)))
	}
}
...
func AdminCompleteTopUp(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil || req.TradeNo == "" {
		common.ApiErrorMsg(c, "Lỗi tham số")
		return
	}
