...
func LogQuota(quota int) string {
	...
	case operation_setting.QuotaDisplayTypeCNY:
		...
		return fmt.Sprintf("¥%.6f hạn ngạch", cny)
	case operation_setting.QuotaDisplayTypeCustom:
		...
		v := usd * rate
		return fmt.Sprintf("%s%.6f hạn ngạch", symbol, v)
	case operation_setting.QuotaDisplayTypeTokens:
		return fmt.Sprintf("%d điểm hạn ngạch", quota)
	default: // USD
		return fmt.Sprintf("＄%.6f hạn ngạch", q/common.QuotaPerUnit)
	}
}
