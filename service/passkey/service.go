...
func BuildWebAuthn(r *http.Request) (*webauthn.WebAuthn, error) {
	...
	if settings == nil {
		return nil, errors.New("Không tìm thấy cài đặt Passkey")
	}
	...
	if originsStr != "" {
		...
		for _, origin := range originList {
			...
			if !settings.AllowInsecureOrigin && strings.HasPrefix(strings.ToLower(trimmed), "http://") {
				return nil, fmt.Errorf("Passkey không cho phép sử dụng Origin không an toàn: %s", trimmed)
			}
...
autoDetect:
	...
	if scheme == "http" && !settings.AllowInsecureOrigin && r.Host != "localhost" && r.Host != "127.0.0.1" && !strings.HasPrefix(r.Host, "127.0.0.1:") && !strings.HasPrefix(r.Host, "localhost:") {
		return nil, fmt.Errorf("Passkey chỉ hỗ trợ HTTPS, truy cập hiện tại: %s://%s, vui lòng cho phép Origin không an toàn trong cài đặt Passkey hoặc cấu hình HTTPS", scheme, r.Host)
	}
	// 优先使用请求的完整Host（包含端口）
	host := r.Host
	...
	if host == "" {
		return nil, fmt.Errorf("Không thể xác định Origin của Passkey, vui lòng chỉ định trong cài đặt hệ thống hoặc cài đặt Passkey. Host hiện tại: '%s', ServerAddress: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New("Passkey chưa cấu hình Origin, không thể suy ra RPID")
	}
	parsed, err := url.Parse(origins[0])
	if err != nil {
		return "", fmt.Errorf("Không thể phân tích Origin của Passkey: %w", err)
	}
