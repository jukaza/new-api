...
func Register(c *gin.Context) {
	...
	if constant.GenerateDefaultToken {
		...
		token := model.Token{
			...
			Name:               cleanUser.Username + " token khởi tạo",
			Key:                key,
...
func generateDefaultSidebarConfig(userRole int) string {
	...
	if err != nil {
		common.SysLog("Tạo cấu hình thanh bên mặc định thất bại: " + err.Error())
		return ""
	}
...
func checkUpdatePassword(originalPassword string, newPassword string, userId int) (updatePassword bool, err error) {
	...
	if !common.ValidatePasswordAndHash(originalPassword, currentUser.Password) && currentUser.Password != "" {
		err = fmt.Errorf("Mật khẩu cũ không chính xác")
		return
	}
	if newPassword == "" {
		return
	...
	case "add_quota":
		...
		case "add":
			...
			model.RecordLogWithAdminInfo(user.Id, model.LogTypeManage,
				fmt.Sprintf("Quản trị viên tăng hạn mức người dùng %s", logger.LogQuota(req.Value)), adminInfo)
		case "subtract":
			if req.Value <= 0 {
				common.ApiErrorI18n(c, i18n.MsgUserQuotaChangeZero)
				return
			...
			model.RecordLogWithAdminInfo(user.Id, model.LogTypeManage,
				fmt.Sprintf("Quản trị viên giảm hạn mức người dùng %s", logger.LogQuota(req.Value)), adminInfo)
		case "override":
			oldQuota := user.Quota
			...
			model.RecordLogWithAdminInfo(user.Id, model.LogTypeManage,
				fmt.Sprintf("Quản trị viên ghi đè hạn mức người dùng từ %s thành %s", logger.LogQuota(oldQuota), logger.LogQuota(req.Value)), adminInfo)
		default:
			common.ApiErrorI18n(c, i18n.MsgInvalidParams)
			return
		}
